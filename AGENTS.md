...
- **UI:** React 19.1.0 + Tailwind CSS 4.x + CSS 변수 기반 디자인 토큰 (라이트/다크 테마)
- **Font:** Pretendard Variable (CDN, 한글 가독성 최우선)
...
- 색상: `src/app/globals.css`의 CSS 변수 토큰 기반. `bg-background`, `text-foreground`, `border-border`, `bg-accent` 등 Tailwind 유틸리티 사용
- 테마: 클래스 기반 다크모드 (`html.dark`). `ThemeToggle` 컴포넌트 + localStorage `theme` 키로 관리
- 카테고리 컬러: `src/utils/categoryStyles.ts`의 `categoryVars()` 인라인 스타일 사용 (라이트/다크 자동 전환)
- 라운드: 카드/모달은 `rounded-3xl`, 버튼/인풋은 `rounded-xl`, 뱃지는 `rounded-full`
